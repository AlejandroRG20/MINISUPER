...
GO
 ...
 GO
  ...
  (1,'Miselanea Reyna Gallo','20202020','Frente al expendio',null)



  -- 1. Usuario Administrador
-- Cuentas y Usuarios
sp_addlogin 'Alejandro3','Camello123','DBVENTASDEMO'
sp_addsrvrolemember 'Alejandro3','sysadmin'
use DBVENTASDEMO
sp_adduser 'Alejandro3','Administrador'
sp_addrolemember 'db_ddladmin','Administrador'

sp_addlogin 'Owen3','Gato123','DBVENTASDEMO'
sp_addsrvrolemember 'Owen3','sysadmin'
use DBVENTASDEMO
sp_adduser 'Owen3','Administrador1'
sp_addrolemember 'db_ddladmin','Administrador1'

-- 2.- Usuarios Comunes
-- 2.1.- Solo Lectura
sp_addlogin 'Clientes','BarAula2023*','SistVBX'
sp_addsrvrolemember 'Clientes','processadmin'
use DBVENTASDEMO
...
-- 2.2.- Solo Escritura
sp_addlogin 'Vendedor','Compra23*','DBVENTASDEMO'
sp_addsrvrolemember 'Vendedor','processadmin'
use DBVENTASDEMO
sp_adduser 'Vendedor','Asistentes'
sp_addrolemember 'db_denydatareader','Asistentes'
sp_addrolemember 'db_datawriter','Asistentes'

deny create table, create view, create procedure
to Asistentes

revoke select on CATEGORIA
to Asistentes
revoke select on CLIENTE
to Asistentes
revoke select on DETALLE_COMPRA
to Asistentes
revoke select on DETALLE_VENTA
to Asistentes
revoke select on NEGOCIO
to Asistentes
revoke select on PERMISO
to Asistentes
revoke select on PRODUCTO 
to Asistentes
revoke select on PROVEEDOR
to Asistentes
revoke select on ROL
to Asistentes
revoke select on USUARIO
to Asistentes
revoke select on VENTA
to Asistentes



-- 2.3.- L/E
sp_addlogin 'Coala','Motoyuca123*','DBVENTASDEMO'
sp_addsrvrolemember 'Coala','processadmin'
use DBVENTASDEMO
sp_adduser 'Coala','Gerente'
...
VALUES ('12345678', 'Usuario', 'usuario@gmail.com', 
        HASHBYTES('SHA2_256', CONCAT(@Contra, CONVERT(VARCHAR(36), @Salt))), @Salt, 1, 1);
...
RETURN (
    WITH Alfabeto AS (
        SELECT CHAR(ASCII('A') + (ROW_NUMBER() OVER (ORDER BY (SELECT NULL)) - 1) % 26) AS Letra
        FROM master.dbo.spt_values
        WHERE TYPE = 'P' AND NUMBER BETWEEN 0 AND 25
    )
    SELECT Letra,
           ROW_NUMBER() OVER (ORDER BY (SELECT NULL)) AS Fila,
           ROW_NUMBER() OVER (ORDER BY (SELECT NULL)) AS Columna
    INTO #TempMatriz
    FROM Alfabeto
    WHERE Letra NOT IN (SELECT DISTINCT SUBSTRING(@clave, Number, 1) FROM master.dbo.spt_values WHERE TYPE = 'P' AND NUMBER BETWEEN 1 AND LEN(@clave))
    
    UNION ALL
    
    SELECT DISTINCT SUBSTRING(@clave, Number, 1) AS Letra,
                    ROW_NUMBER() OVER (ORDER BY (SELECT NULL)) AS Fila,
                    (LEN(@clave) + ROW_NUMBER() OVER (ORDER BY (SELECT NULL))) % 5 + 1 AS Columna
    FROM master.dbo.spt_values
...
AS
BEGIN
    DECLARE @matriz TABLE (Letra NVARCHAR(1), Fila INT, Columna INT);
    INSERT INTO @matriz
